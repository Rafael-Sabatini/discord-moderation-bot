...
const CDH_LINK_REGEX = /https?:\/\/(cdh\.|cdn\.discordapp\.com)/i;
...
module.exports = {
  ...
  async execute(client, message) {
    ...
    // Check for CDH or CDN links in message
      ...
      try {
        ...
        if (!hasTrustedRole) {
          ...
          await logAction(message.guild, "messages", {
            ...
            reason: "Blocked CDH/CDN image link - user not trusted",
          });

          // Send DM to user with timeout to prevent hanging
          try {
            const dmPromise = message.author.send(
              `⚠️ Your message in **${message.guild.name}** was deleted because it contained a CDH or Discord CDN image link. These types of links are only allowed for trusted members.`
            );
        ...
        if (error.code === 10008) {
          ...
          console.error("[messageCreate] Error handling CDH/CDN link:", error);
        }
      }
    }
  },
